...
export default function BairroViewRico({
  bairro, slug, zona, vizinhos, descricao, referencias, depoimentos, atendimentos = 200
}: BairroRicoProps) {
    ...
    <div className="pb-20">
        ...
        <div className="container mx-auto px-4">
          ...
          <div className="max-w-3xl">
            ...
            <h1 className="text-3xl md:text-5xl font-extrabold text-white leading-tight mb-4">
              Ar Condicionado em {bairro}
            </h1>
            <p className="text-blue-100/80 text-lg mb-6">
              Instalação, manutenção, limpeza e conserto em {bairro} — com técnicos que moram e trabalham aqui perto.
            </p>

            <div className="flex flex-col gap-2 mb-8 max-w-xl">
              {[
                `Sede própria em ${bairro} — R. Santa Cruz, 2187`,
                'Visita técnica gratuita — orçamento no mesmo dia',
                'Nota 4.9 ⭐ no Google · Mais de 24 avaliações reais',
              ].map((item) => (
            ...
            <div className="flex flex-col sm:flex-row gap-4">
              <a href={`${WA_BASE}${waText}`} rel="noopener noreferrer"
            ...
            <div className="flex flex-wrap gap-6 mt-8 pt-8 border-t border-white/10">
              {[
                { num: `+${atendimentos}`, label: `atendimentos em ${bairro}` },
                { num: '4.9★', label: 'nota no Google' },
                { num: '7 anos', label: 'de experiência' },
                { num: '2h', label: 'tempo de resposta' },
              ].map((item) => (
                <div key={item.label}>
                  <div className="text-2xl font-extrabold text-white">{item.num}</div>
                  <div className="text-blue-100/50 text-xs">{item.label}</div>
                </div>
              ))}
            </div>
          </div>
            ...
            <div>
              ...
              {referencias.length > 0 && (
                <div className="flex flex-wrap gap-2 mt-3">
                  {referencias.map((ref) => (
                    <span key={ref} className="bg-white border border-slate-200 text-slate-600 text-xs px-3 py-1 rounded-full">
                      📍 {ref}
                    </span>
                  ))}
                </div>
        ...
        <div className="container mx-auto px-4">
          <div className="text-center mb-10">
            <h2 className="text-2xl md:text-3xl font-extrabold text-primary mb-2">
              Seu ar condicionado em {bairro} está com algum desses problemas?
            </h2>
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 gap-4 max-w-3xl mx-auto mb-8">
            {problemas.map((p) => (
              <div key={p.titulo} className="bg-slate-50 border border-slate-100 rounded-2xl p-4 flex items-start gap-3">
                ...
                <div>
                  <div className="font-bold text-primary text-sm">{p.titulo}</div>
          ...
          <div className="text-center">
            <a href={`${WA_BASE}${encodeURIComponent(`Olá! Meu ar condicionado está com problema em ${bairro}. Podem me ajudar?`)}`}
              rel="noopener noreferrer"
              className="inline-flex items-center gap-2 bg-success text-white font-bold py-3 px-8 rounded-xl shadow hover:-translate-y-0.5 transition-all">
              <WhatsAppIcon size={18} />
              Resolver agora — {bairro}
            </a>
          </div>
        </div>
      </section>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
            {servicos.map((s) => (
              <Link key={s.tag} href={s.href}
                className="bg-white rounded-2xl border border-slate-100 p-6 hover:border-primary hover:shadow-xl hover:-translate-y-1 transition-all duration-300 group block">
      ...
      <section className="py-10 bg-primary-light">
        <div className="container mx-auto px-4 flex flex-col md:flex-row items-center justify-between gap-6">
          ...
          <a href={`${WA_BASE}${waText}`} rel="noopener noreferrer"
            className="inline-flex items-center gap-2 bg-success text-white font-bold py-3 px-8 rounded-xl shadow whitespace-nowrap hover:-translate-y-0.5 transition-all">
            <WhatsAppIcon size={18} />
            Falar no WhatsApp
          </a>
        </div>
      ...
      <section className="py-14 bg-white">
        <div className="container mx-auto px-4 max-w-4xl">
          <h2 className="text-2xl md:text-3xl font-extrabold text-primary mb-2 text-center">
            Por que a New Clima é a escolha certa em {bairro}?
          </h2>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {diferenciais.map((d) => (
              <div key={d.titulo} className="bg-slate-50 p-6 rounded-2xl border border-slate-100 text-center">
                <div className="w-12 h-12 bg-primary/10 rounded-xl flex items-center justify-center text-primary-light mx-auto mb-4">
                  {d.icon}
                </div>
      ...
      <section className="py-14 bg-slate-50">
        <div className="container mx-auto px-4 max-w-4xl">
          <h2 className="text-2xl font-extrabold text-primary mb-2 text-center">
            O que dizem os clientes da New Clima
          </h2>
          ...
          <div className="text-center mt-6">
            <a href="https://maps.app.goo.gl/kHeynjnXoG943iG3A" target="_blank" rel="noopener noreferrer"
              className="inline-flex items-center gap-2 text-primary-light font-semibold text-sm hover:underline">
      ...
      <section className="py-10 bg-slate-50">
        <div className="container mx-auto px-4">
          ...
          <div className="flex flex-wrap gap-3">
            {vizinhos.map((v) => (
              <div key={v} className="flex items-center gap-2 bg-white border border-slate-200 px-4 py-2 rounded-full text-sm font-semibold text-primary-light">
                <MapPin size={12} /> {v}
              </div>
            ))}
            <Link href="/ar-condicionado-sao-paulo"
